...
namespace Lidgren.Network
{
	public class NetDESEncryption : INetEncryption
	{
		...
		static NetDESEncryption()
		{
			...
			KeySizes[] legalKeySizes = dESCryptoServiceProvider.LegalKeySizes;
			...
			m_keysizes = list;
			list = new List<int>();
			KeySizes[] legalBlockSizes = dESCryptoServiceProvider.LegalBlockSizes;
		...
		public bool Encrypt(NetOutgoingMessage msg)
		{
			...
			{
				DESCryptoServiceProvider dESCryptoServiceProvider = new DESCryptoServiceProvider();
				dESCryptoServiceProvider.KeySize = m_bitSize;
				dESCryptoServiceProvider.Mode = CipherMode.CBC;
				using (DESCryptoServiceProvider dESCryptoServiceProvider2 = dESCryptoServiceProvider)
				{
					using (ICryptoTransform transform = dESCryptoServiceProvider2.CreateEncryptor(m_key, m_iv))
					{
		...
		public bool Decrypt(NetIncomingMessage msg)
		{
			try
			{
				...
				using (DESCryptoServiceProvider dESCryptoServiceProvider2 = dESCryptoServiceProvider)
				{
					using (ICryptoTransform transform = dESCryptoServiceProvider2.CreateDecryptor(m_key, m_iv))
					{
						using (MemoryStream memoryStream = new MemoryStream())
						{
							using (CryptoStream cryptoStream = new CryptoStream(memoryStream, transform, CryptoStreamMode.Write))

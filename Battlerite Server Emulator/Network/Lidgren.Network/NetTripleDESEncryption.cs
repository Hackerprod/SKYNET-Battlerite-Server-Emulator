...
namespace Lidgren.Network
{
	public class NetTripleDESEncryption : INetEncryption
	{
		...
		static NetTripleDESEncryption()
		{
			...
			KeySizes[] legalKeySizes = tripleDESCryptoServiceProvider.LegalKeySizes;
			...
			m_keysizes = list;
			list = new List<int>();
			KeySizes[] legalBlockSizes = tripleDESCryptoServiceProvider.LegalBlockSizes;
		...
		public bool Encrypt(NetOutgoingMessage msg)
		{
			...
			{
				TripleDESCryptoServiceProvider tripleDESCryptoServiceProvider = new TripleDESCryptoServiceProvider();
				tripleDESCryptoServiceProvider.KeySize = m_bitSize;
				tripleDESCryptoServiceProvider.Mode = CipherMode.CBC;
				using (TripleDESCryptoServiceProvider tripleDESCryptoServiceProvider2 = tripleDESCryptoServiceProvider)
				{
					using (ICryptoTransform transform = tripleDESCryptoServiceProvider2.CreateEncryptor(m_key, m_iv))
					{
		...
		public bool Decrypt(NetIncomingMessage msg)
		{
			try
			{
				...
				using (TripleDESCryptoServiceProvider tripleDESCryptoServiceProvider2 = tripleDESCryptoServiceProvider)
				{
					using (ICryptoTransform transform = tripleDESCryptoServiceProvider2.CreateDecryptor(m_key, m_iv))
					{
						using (MemoryStream memoryStream = new MemoryStream())
						{
							using (CryptoStream cryptoStream = new CryptoStream(memoryStream, transform, CryptoStreamMode.Write))
